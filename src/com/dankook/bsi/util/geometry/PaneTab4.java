...
public class PaneTab4 {
	...
	private CategoryDataset createDataset_tab4() throws UnsupportedEncodingException {
		...
		for (int i = 0; i < 12; i++) {
			dataset.addValue(_model.getInfo().getQ_c_f_elec()[i], type[0], series[i]);
			dataset.addValue(_model.getInfo().getQ_c_f_gas()[i], type[1], series[i]);
			dataset.addValue(_model.getInfo().getQ_c_f_local()[i], type[2], series[i]);
		}
